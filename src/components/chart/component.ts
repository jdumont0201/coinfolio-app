...
export class AppChartComponent extends CheckValid {
    ...
    paintYAxis() {
        ...
        this.arranger.yAxis.forEach((li) => {
            ...
            this.drawer.drawText(this.arranger.W - 5, Math.round(li.val - 12), li.text, opt.yAxis.grid.textColor, "right", null);
        })
    }
    ...
    paintXAxis() {
        ...
        this.arranger.xAxis.forEach((li) => {
            //console.log("paint",li.val, 0, li.val, this.arranger.H , this.arranger.options.chart.MB);
            D.drawLine("xaxis-" + li.val, li.val, 0, li.val, this.arranger.H  -this.arranger.options.xAxis.height , opt.xAxis.grid.strokeWidth, opt.xAxis.grid.color);
            D.drawText(li.val , Math.round(xAxisY + opt.xAxis.height / 2) - 5, li.text, "rgb(255,255,255)", "center", null);
        })
    }
        ...
        if (this.Data.indicators) {
            if (this.Data.indicators.SMAScaled) {
                ...
                for (let i = this.arranger.idxMin; i <= this.arranger.idxMax - 1; ++i) {
                    ...
                    if (ind[i] && ind[i + 1] && i > this.arranger.options.indicators.onchart.sma.period) {
    ...
    paintMarker() {
        ...
        this.drawer.drawText(this.arranger.W - 5, L.flipped.fc - 5, L.raw.c.toString(), "rgba(0,0,0,1)", "right", null);
    }
