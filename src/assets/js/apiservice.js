...
onmessage = function(e) {
    ...
    for(var i=0;i<e.data.headers.headers.length;++i){
      ...
      //  console.log(" --> sww header",h.title,h.value)
        xhr.setRequestHeader(h.title, h.value)
    }
    for(var i=0;i<e.data.headers.lazyUpdate.length;++i){
        var h=e.data.headers.lazyUpdate[i]
        //console.log(" --> sww header",h.name,h.value)
